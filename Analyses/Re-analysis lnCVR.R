...
#--------------------------------------------------------#
    ...
    source("./New Analyses/EffectSizeFunctions.3.0.R")

    # Load data. Working directory assumed to be the project folder main directory.
    data<-read.csv("./New Analyses/Clean_Data.csv")
    head(data)
...
REMA<-rma.mv(yi = lnCVR, V = V, random=~1|EffectID, data=data)
MLMA<-rma.mv(yi = lnCVR, V = V, random=~1|StudyNo/EffectID, data=data)
MLMA<-rma.mv(yi = lnCVR, V = V, random=list(~1|StudyNo, ~1|EffectID), data=data) # Notation I would have used.
